...
function truncate(msg, maxLength) {
    if (!msg || !maxLength) {
        return msg;
    }
    else if (msg.length > maxLength) {
      return msg.substr(0,maxLength) + '...';
    } else {
      return msg;
    }
}
 ...
 *      - `maxLength`: integer (default undefined). Set to an integer greater than zero
 *         to limit the length of the message part of a log line.
...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (options.maxLength && (!Number.isInteger(options.maxLength) || options.maxLength < 1)) {
        throw new TypeError('invalid options.maxLength: must be an integer greater than zero')
    }
...
Logger.prototype._emit = function (rec, noemit) {
    ...
    if (noemit)
        return truncate(str, this.maxLength);
...
function mkRecord(log, minLevel, args) {
    ...
    rec.msg = truncate(format.apply(log, msgArgs), log.maxLength);
    if (!rec.time) {
        rec.time = (new Date());
    }
